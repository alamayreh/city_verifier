...
class SiameseNetwork(pl.LightningModule):
    def __init__(self, margin, learning_rate, resize, imageFolderTrain, imageFolderTest, batch_size,num_workers, should_invert):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        self.siamese_dataset_train = SiameseNetworkDataset(imageFolderDataset=self.DatasetFolder,
                                                           transform=self.transform, should_invert=self.should_invert)
        self.siamese_dataset_test = SiameseNetworkDataset(imageFolderDataset=self.DatasetFolder_testing,
                                                          transform=self.transform, should_invert=self.should_invert)

    def train_dataloader(self):
        return DataLoader(self.siamese_dataset_train, num_workers=self.num_workers,batch_size=self.batch_size)

    def test_dataloader(self):
        return DataLoader(self.siamese_dataset_test,num_workers=self.num_workers, batch_size=self.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imageFolderTrain', default='dataset/cities/training')
    parser.add_argument('--imageFolderTest', default='dataset/cities/test')
    ...
    model = SiameseNetwork(margin=args.margin, learning_rate=args.learning_rate, resize=args.resize, imageFolderTrain=args.imageFolderTrain,
                           imageFolderTest=args.imageFolderTest, batch_size=args.batch_size, num_workers=args.num_workers, should_invert=args.should_invert)
    trainer = pl.Trainer(
        gpus=args.gpus, max_epochs=args.pretrain_epochs, progress_bar_refresh_rate=5)
    trainer.fit(model)
    trainer.save_checkpoint("models/siamese_face_recognition.ckpt")
    trainer.test()
