...
#python3 analysis_results.py --test_city Moscow --database_city Tokyo
...
def Probabily_365(Probabily_365):

    image_prob_str =((Probabily_365)[1:])[:-1].split(' ')
...
def distance_euclidean(Probabily_365_test,Probabily_365_base):

    test_prob = Probabily_365(Probabily_365_test)
    base_prob = Probabily_365(Probabily_365_base)

    eDistance = math.dist(base_prob, test_prob)

    return (eDistance + 0.01)     

def distance_cos(Probabily_365_test,Probabily_365_base):
...
if __name__ == '__main__':
    ...
    # Read similarity  
    test_simialrity_db           =  pd.read_csv(str(join(args.image_csv_test, args.test_city)) + '.csv')
    test_simialrity_db.rename(columns={'IMG_ID': 'IMG_ID_test', 'Probabily_365': 'Probabily_365_test'}, inplace=True)


    database_simialrity_db = pd.read_csv(str(join(args.database_csv, args.database_city)) + '.csv')
    database_simialrity_db.rename(columns={'IMG_ID': 'IMG_ID_data_base', 'Probabily_365': 'Probabily_365_base'}, inplace=True)
    ...
    #df['eDistance'] = df.apply(lambda x: distance_euclidean(x.Probabily_365_base, x.Probabily_365_test), axis=1)
    #df['cDistance'] = df.apply(lambda x: distance_cos(x.Probabily_365_base, x.Probabily_365_test), axis=1)

    df.drop(columns=['IMG_ID_data_base','Probabily_365_base','Probabily_365_test'],inplace=True)
    ...
    print("####################################################################")
    #print(f"Based on votes_sigmoid hard                              : {same_db_sig[0].size}")
    print(f"Based on votes_sigmoid soft                              : {same_db_sig_soft[0].size / len_images}")
    #print(f"Based on votes_sigmoid > 0.90 and < 0.01                 : {same_thr}")
    print("####################################################################")
    ...
    #print(f"Based on probablity_same_similarity Top 5 Euclidean      : {same_e_distance[0].size}")
    #print(f"Based on probablity_same_similarity Top 5 Cosine         : {same_c_distance[0].size}")
    print("####################################################################")
